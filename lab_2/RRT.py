...
def find_closestNode(V, xt):
    ...
    weighted_dist = ((V[0].state[0] - xt[0]) / (2 * math.pi * R * wmax)) ** 2 + ((V[0].state[1] - xt[1]) / (2 * math.pi * R * wmax)) ** 2 + ((V[0].state[2] - xt[2]) / (4 * math.pi * R * wmax / W)) ** 2

    for point in V:
        dist = ((point.state[0] - xt[0]) / (2 * math.pi * R * wmax)) ** 2 + ((point.state[1] - xt[1]) / (2 * math.pi * R * wmax)) ** 2 + ((point.state[2] - xt[2]) / (4 * math.pi * R * wmax / W)) ** 2
...
def generate_trajectory(xi, xt):
    ...
    if remain_time < req_dis_time :       # not enough time to reach the target after rotation
        dist = remain_time * C
        end = [xi[0] + dist * math.cos(angle), xi[1] + dist * math.sin(angle), end[2]]
        trajectory.append(end)
        # print(trajectory)
    else:                            # with enough time to reach the target after rotation
        end = [xt[0], xt[1], end[2]]
        trajectory.append(end)
        # print(trajectory)
...
# whether two lines are intersection
...
# whether two rectangles are intersection
...
# Problem 2(d)
# Combine the above to implement an RRt planner generating a trajectory from a specified initial state to a desired goal region. Visualize the evolution of the RRT.
