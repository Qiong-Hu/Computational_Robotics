...
def generate_trajectory(policy, s0, pe = 0, show = True):
    ...
    if show:
        ...
        plt.savefig('./img-'+time.strftime("%H%M%S", time.localtime())+'.jpg')
...
def policy_iteration_timing():
    ...
    # s0 = (1, 6, 6)
    trajectory = generate_trajectory(optimal_policy, s0, pe = 0, show = False)
    end = time.time()
